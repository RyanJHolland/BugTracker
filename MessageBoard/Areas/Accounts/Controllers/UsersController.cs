...
namespace TicketTracker.Areas.Accounts.Controllers
{
	public class UsersController : BaseController
	{
		...
		private static Dictionary<string, string> ClaimTypeToName;
		private static Dictionary<string, string> ClaimTypeToAlias;
		...
		private static string ClaimsGet(Dictionary<string, string> claimsDict, string key)
		{
		...
		private async Task<IdentityResult> SaveUserAsync(IdentityUser user, List<string> userRoleNames, List<IdentityUserClaim<string>> userClaims)
		{
			...
			if (userManager.SupportsUserRole)
			{
				...
				// Synchronize with stored claims:
				var storedUserRoleNames = context.Roles
					.Where(r => context.UserRoles
						.Where(ur => ur.UserId == storedUser.Id)
						.Select(ur => ur.RoleId)
						.Contains(r.Id))
					.Select(r => r.Name)
					.ToList();
